...
{
    cmds_ = cmds; // IMPORTANT: otherwise cmds_ is default-empty unless you called setCommands()
    ...
    if (cfg.showBody)       drawBodyPanel(bodyState);
    if (cfg.showCamera)     drawCameraPanel(camState);
    if (cfg.showController) drawControllerPanel(ctrlState);
...
void UI::drawControllerPanel(const UIControllerState& s)
{
    if (ImGui::Begin("Controller")) {
        ...
        if (ImGui::SliderFloat("Move speed", &move, 0.0001f, 0.01f, "%.5f")) {
