...
Force outputs were limited on the host side to a predefined maximum, and the embedded controller applied an additional clamp to ensure commands remained within safe bounds. Testing confirmed that:

\begin{itemize}
	\item host-side force commands were correctly limited
	\item transmitted torque commands were further bounded by the embedded controller
	\item actuator output remained within safe and predictable limits
\end{itemize}
...
A slew-rate limiter was implemented to reduce high-frequency oscillations and improve stability during rapid contact transitions. Testing verified that:
...
\begin{itemize}
	...
	\item overall system stability improved during interaction
\end{itemize}
...
The embedded controller response confirmed that:

\begin{itemize}
	\item the watchdog triggered reliably when command updates ceased
	\item commanded torque was removed within the intended timeout window
	\item the device returned to a safe passive state
\end{itemize}
