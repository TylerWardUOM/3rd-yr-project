...
\begin{figure}[H]
	...
	\caption{Final assembled 2-DOF haptic interface prototype. The system consists of a planar two-link mechanism with BLDC gimbal motors, magnetic encoders, and a belt-driven base joint for torque amplification.}
...
Host--device communication uses fixed-format binary packets over a USB virtual serial link at high update rate (target 1\,kHz exchange). Two packet families are used: device state packets (firmware \(\rightarrow\) host), containing sequence number, MCU timestamp, and joint angles; and torque command packets (host \(\rightarrow\) firmware), containing command sequence, referenced state sequence, and commanded joint torques.
...
%TODO: Swim lane diagram in apendix
%\begin{figure}[htbp]
%	\centering
%	\fbox{%
...
\]
where \(\mathbf{x}_{p},\mathbf{x}_{t}\) are proxy and tool positions and \(\mathbf{v}_{p},\mathbf{v}_{t}\) are corresponding velocities. In the implemented configuration, \(K=500\,\mathrm{N\,m^{-1}}\), \(M=0.02\,\mathrm{kg}\), and \(D = 0.7\cdot 2\sqrt{KM}\).
...
Accordingly, the force command is generated using a deliberately conservative coupling configuration, with damping and actuator-side limiting used to reduce abrupt force changes during contact transitions. The stability implications of this design are discussed in Section~\ref{sec:discussion_stability}.
