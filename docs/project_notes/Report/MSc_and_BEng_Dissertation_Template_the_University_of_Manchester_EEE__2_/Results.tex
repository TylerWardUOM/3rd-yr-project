...
Packet loss was more pronounced at lower transmission rates, a counterintuitive finding that may reflect the influence of host-side scheduling on USB buffer flushing behaviour; this is discussed further in Section~\ref{sec:discussion_communication_latency}.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{images/Packet timing jitter vs requested rate.pdf}
	\caption{
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{images/CDF of round-trip latency.pdf}
	\caption{
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{images/Matched host-side end-to-end latency distribution.pdf}
	\caption{
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{images/Raw parsed state packet inter-arrival distribution.pdf}
	\caption{
		Distribution of host-side inter-arrival time between consecutively parsed state packets. 
		A dominant peak near 0~ms and a smaller concentration around 14--16~ms show non-uniform host-side packet timing.
	}
...
Taken together, the embedded controller generated packets at a regular rate, while host-side packet timing was non-uniform. Host-side processing latency nevertheless remained low.
...
Simulation validation was performed as described in Section~\ref{sec:method_simulation}, 
with actuator output disabled.

\subsubsection{Virtual Wall Model Response} \label{sec:results_virtual_wall}
...
\subsubsection{Stable Contact Behaviour (Virtual Wall Interaction)} \label{sec:results_stable_contact_hardware}

This test represents the physical realisation of the virtual wall interaction previously validated in Section~\ref{sec:results_virtual_wall}.
...
Some low-level vibration and noise were present during contact, particularly under high stiffness settings; however, this did not lead to instability and was reduced through the combined use of damping and torque rate limiting. The system remained responsive to user input while maintaining a stable resistive force, demonstrating stable contact interaction under real-time conditions despite communication variability.
...
Although stable contact behaviour was achieved, the rendered force did not exhibit perfectly rigid characteristics. The interaction was observed to have a degree of compliance, with force increasing gradually rather than instantaneously. This behaviour reflects the combined effect of the torque slew-rate limiter, actuator torque capability limitations, control fidelity, and transmission behaviour, which are discussed further in Sections~\ref{sec:discussion_hardware_limitations} and~\ref{sec:discussion_design_tradeoffs}.
...
An additional observation was made when the device was released near a virtual surface: when positioned close to the surface, the end-effector could remain supported by the rendered contact force, behaving similarly to a physical object resting against a rigid boundary, while release further from the surface led to free motion until contact was re-established.

This behaviour demonstrates that the system is capable of producing physically intuitive constraint forces, while also reflecting the finite stiffness and force limits of the actuator system. The transition between free motion and constrained interaction was smooth and did not result in instability. The guidance behaviour was effective but not perfectly rigid, with some compliance and variation in constraint force observed during motion, consistent with the limited actuator stiffness and partial actuation available in the final hardware configuration.
