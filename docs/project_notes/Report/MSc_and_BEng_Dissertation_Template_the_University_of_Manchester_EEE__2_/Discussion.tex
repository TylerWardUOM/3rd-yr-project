...
stability under delay, and hardware limitations. It analyses how these factors interact and the resulting trade-offs in achievable haptic performance.
...
The increased packet loss observed at low transmission rates is likely attributed to host-side scheduling and USB buffering behaviour. At lower rates, packets are more likely to be grouped into bursts, increasing the likelihood of missed reads or buffer overrun in the host polling loop.

The burst-like packet processing observed on the host suggests that packets are delivered in chunks rather than at uniform intervals. This behaviour is consistent with USB CDC buffering and operating system scheduling, which introduce non-uniform delivery despite regular transmission from the embedded system.
...
\subsection{Stability of the Haptic Interaction} \label{sec:discussion_stability}
...
Firstly, snapshot-based communication channels ensured that the haptic loop operated on the most recent available state rather than queued historical samples. This design choice limited the destabilising effect of burst delivery by preventing stale data from accumulating in the feedback path, thereby avoiding stale-state feedback that would otherwise increase the effective delay and reduce stability margins. Although formal passivity conditions were not enforced, the combination of damping, reduced stiffness, and actuator-side rate limiting can be interpreted as introducing dissipation to counter delay-induced energy injection. This places the implemented design within the class of passivity-related stability concerns identified by Colgate and Schenkel \cite{colgatePassivityClassSampleddata1997}, while remaining an empirical stabilisation strategy rather than a formally passive controller.
...
The design of the system reflects a set of trade-offs between stability, responsiveness, and achievable force fidelity. While damping and torque rate limiting were effective in ensuring stable interaction, they reduce perceived stiffness and responsiveness. In particular, rate limiting of actuator torque results in a finite rise time when contact forces are applied, leading to a short delay between initial contact and the development of the full resistive force. This reduces the sharpness of hard-contact interactions such as collisions with rigid virtual walls, as observed during the hardware validation in Section~\ref{sec:results_stable_contact_hardware}.
...
Project management in this work was not followed as a rigid, pre-defined process, but evolved as technical challenges emerged. While an initial project plan was created to outline development stages, the Gantt chart included in Appendix~\ref{app:gantt} represents the final execution of the project. Development of the host software began prior to the official project start, with initial work on OpenGL-based rendering and interaction undertaken in late August, providing a foundation for subsequent system integration.
...
Risk management was primarily reactive, although key risks were identified early (Appendix~\ref{app:risk_assesment}), including control instability, communication failure, and actuator limitations. As these arose, mitigation strategies such as torque limiting, slew-rate limiting, and watchdog safety mechanisms were introduced.
...
The project required significant self-directed learning, documented in the CPD log (Appendix~\ref{app:cpd_log}), including OpenGL rendering, haptic rendering literature, BLDC motor control with SimpleFOC, PhysX simulation integration, hardware integration, and technical report writing. These activities supported key design decisions such as the use of proxy-based rendering, torque-mode actuation, and the integration of the simulation and visualisation framework.
